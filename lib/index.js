...
var DEFAULT_DELEGATE = {
  ...
  create: function(inputPath, outputPath, relativePath) {
    ...
    rename: function(inputPath, outputPath, relativePath) {
...
FSTree.prototype.calculatePatch = function(otherFSTree, isEqual) {
  ...
  while (i < ours.length && j < theirs.length) {
    ...
    if (x.relativePath < y.relativePath) {
        ...
        var index = 0;
        var foundfile = 0;
        var ourfilename = x.relativePath.split("/")[x.relativePath.split("/").length-1];
        for (var k = 0; k < theirs.length; k++)  {
            var theirfilename = theirs[k].relativePath.split("/")[theirs[k].relativePath.split("/").length-1];
            if(ourfilename === theirfilename) {
                found = true;
                index = k;
                foundfile = theirfilename;
            }
        }

        if(found)  {
            // FOUND MOVED FILE IN DESTINATION
            console.log("ignoring and prepping for move: " + foundfile);
            renamefiles.push(foundfile);

        }   else    {
            // SOURCE HAS REMOVED A FILE
        ...
        var ignorefilename = 0;
        var ourfilename1 = x.relativePath.split("/")[x.relativePath.split("/").length-1];

        for (var g = 0; g < renamefiles.length - 1; g++)   {
            console.log(renamefiles[g], ourfilename1);
            if (renamefiles[g] === ourfilename1)   {
                ignoreflag = true;
                ignorefilename = ourfilename1
            }
        }

        // FILE FOUND ELSEWHERE IN DESTINATION
        if(ignoreflag) {
            // renames.push(renameCommand(theirs[index], x.basePath));
            console.log("Found file, skipping command: " + ignorefilename);
        } else  {
...
function renameCommand(entry, basePath) {
  return ['rename', entry.relativePath, entry]
}
