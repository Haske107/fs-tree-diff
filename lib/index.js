...
FSTree.prototype.calculatePatch = function(otherFSTree, isEqual) {
  ...
  while (i < ours.length && j < theirs.length) {
    ...
    if (x.relativePath < y.relativePath) {
      ...
      var found = false;
      var ourfilename = ours.relativePath.split("\\")[ours.relativePath.split("\\").length-1];
      console.log(ourfilename);
      for (var item in theirs)  {
          var theirfilename = item.relativePath.split("\\")[item.relativePath.split("\\").length-1];
